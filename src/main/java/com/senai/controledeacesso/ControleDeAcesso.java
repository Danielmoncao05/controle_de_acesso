...
ControleDeAcesso {
    ...
    private static void menuPrincipal() {
        ...
        do {
            String menu = """
                    ...
                    |       6- Listar registro de acesso por usuário        | 
                    |       7- Limpar registros de usuário                  | 
                    |       8- Sair                                         |
                    _________________________________________________________
                    """;
    ...
    private static void listarRegistroDeAcesso() {
        System.out.println("Escolha o usuário que deseja listar os registros de acesso");

        if (idUsuarioRecebidoPorHTTP == 0) {
            // Exibe a lista de usuários para o administrador escolher
            for (String[] usuario : matrizCadastro) { // transformou a matriz num veor e repartiu em colunas
                System.out.println(usuario[0] + " - " + usuario[2]); // Exibe ID e Nome do usuário
            }
        }
    }

    private static void limparRegistroDeAcesso() {
    ...
    private static void carregarRegistroDeAcesso() {
        try (BufferedReader reader = new BufferedReader(new FileReader(registroDeAcesso))) {
            String linha;
            StringBuilder conteudo = new StringBuilder();
    ...
    private static void salvarRegistroDeAcesso() {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(registroDeAcesso))) {
    ...
    private static void verificarEstruturaDeDiretorios() {
        ...
        //Verifica se o arquivo registroDeAcesso.txt existe, caso contrário, cria
        if (!registroDeAcesso.exists()) {
            try {
                if (registroDeAcesso.createNewFile()) {
                ...
                System.out.println("Erro ao criar o arquivo registroBancoDeDados.txt: " + e.getMessage());
            }
        }
